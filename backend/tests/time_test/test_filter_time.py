...
class TestFilterSearchResponseTime(unittest.TestCase):
    def setUp(self):
    ...
    def test_filter_search_time(self):
        start_time = time.time()
        res = self.app.get('/main_sys/filter?title=art', headers=header)
        end_time = time.time()

        duration = end_time - start_time
        print(f"Filter Search runtime: {duration} seconds")
