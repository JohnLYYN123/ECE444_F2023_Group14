...
class TestEventDetailResponseTime(unittest.TestCase):
    def setUp(self):
    ...
    def test_event_detail_time(self):
        start_time = time.time()
        res = self.app.get('/detail/view_detail?event_id=2', headers=header)
        end_time = time.time()

        duration = end_time - start_time
        print(f"Event detail runtime: {duration} seconds")
